...
Generation BoundedEvolution::evolve(const Generation &generation)
{
...
bool BoundedEvolution::isCellAlive(Generation generation, int row, int col) const
{
   if (!generation.isValidCell(row, col))
      return false;
   return generation.cell(row, col);
}

int BoundedEvolution::livingNeighbors(Generation generation, int row, int col) const
{
   int living = 0;
